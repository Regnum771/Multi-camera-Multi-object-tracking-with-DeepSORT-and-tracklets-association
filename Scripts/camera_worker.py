...
class CameraWorker:
    #handles detection, tracking, tracklets, global ID assignment and visualization per camera.
            ...
            while not self.shutdown_event.is_set():
                ...
                for track in tracks:
                    ...
                    if track_id not in self.track_to_gid:
                        gid, sim_score = self.global_id_store.match_tracklet(
                            tracklet_embedding, exclude_gids=used_gids
                        )
                        if gid is None:
                        ...
                        sim_score = max(
                            cosine_similarity(tracklet_embedding, e)
                            for e, _ in self.global_id_store._store[gid].embedding_buffer
                        )

                    used_gids.add(gid)

                    # Update Global ID with frame embedding
                    ...
                    #store detection
                    #confirmed track default to 1.0 confidence
                    track_conf = track.last_detection.confidence if hasattr(track, "last_detection") else 1.0
